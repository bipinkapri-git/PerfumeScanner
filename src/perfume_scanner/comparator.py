...
def clean_price(price_str: str) -> float:
    ...
    for m in matches:
        try:
            val = float(m)
            if val >= 50.0:  # Minimum realistic price in INR/USD
                return val
        except ValueError:
            pass
...
def process_and_compare_deals(deals: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    """
    if not deals:
        return {"sorted_deals": [], "cheapest_deal": None, "price_range": (0.0, 0.0)}

    processed_deals = []
    for deal in deals:
        cleaned_val = clean_price(deal.get("price_str", ""))
        if cleaned_val == float("inf"):
            continue
        deal_copy = deal.copy()
    ...
    if valid_deals:
        ...
        for deal in sorted_deals:
            if (
                deal.get("retailer") == cheapest_deal["retailer"]
                and deal["price_val"] == cheapest_deal["price_val"]
            ):
